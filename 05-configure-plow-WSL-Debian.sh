...
echo -e "$ wsl.exe --version | iconv -f UTF-16LE -t UTF-8\n"
wsl.exe --version | iconv -f UTF-16LE -t UTF-8
echo -e "\n$ uname -a\n"
uname -a
WSL_KERNEL=$(wsl.exe --version | iconv -f UTF-16LE -t UTF-8 | grep -i "Kernel \
version" | sed 's/^Kernel version: //' | grep -oE "^[0-9.]+")
