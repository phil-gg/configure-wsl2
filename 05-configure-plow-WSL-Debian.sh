...
USER_DIR="${HOME}/.local/share/plasma/shells/org.kde.plasma.desktop/contents\
/updates"
SKEL_DIR="/etc/skel/.local/share/plasma/shells/org.kde.plasma.desktop/contents\
/updates"
TASKBAR_FILENAME="plow-taskbar.js"
USER_FILE="${USER_DIR}/${TASKBAR_FILENAME}"
SKEL_FILE="${SKEL_DIR}/${TASKBAR_FILENAME}"
...
if [ ! -f "${SVG_FILE}" ] || \
   ...
   [ ! -f "${USER_FILE}" ] || \
     ! cmp -s <(printf "%s" "${TASKBAR_TEXT}") "${USER_FILE}" || \
   [ ! -f "${SKEL_FILE}" ] || \
     ! cmp -s <(printf "%s" "${TASKBAR_TEXT}") "${SKEL_FILE}"; then
...
echo -e "$ printf \"%s\" \"\${TASKBAR_TEXT}\" | sudo tee ${SKEL_FILE} > \
/dev/null"
printf "%s" "${TASKBAR_TEXT}" | sudo tee "${SKEL_FILE}" > /dev/null

echo -e "$ printf \"%s\" \"\${TASKBAR_TEXT}\" | tee ${USER_FILE} > /dev/null"
